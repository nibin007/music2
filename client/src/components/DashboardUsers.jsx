...
function DashboardUsers() {
  ...
  return (
    <div className='w-full p-4 flex items-center justify-center flex-col'>
      ...
      <div className='relative w-full py-12 min-h-[400px] overflow-x-scroll my-4 flex flex-col items-center justify-start p-4 border border-gray-200 rounded-md gap-3'>
    <div className='absolute top-4 left-4'>
      <p className='text-sm font-semibold '>
      count: <span className='text-xl font-bold text-text'> {allUsers?.length} </span>
    </p>
    </div>
...
return(
...

 {
  data._id !==user?.user._id && (
    <motion.div  whileTap={{scale:0.75}} className='absolute left-4 w-8 h-8 rounded-md flex items-center justify-center bg-gray-200 ' onClick={()=>setisUserDeleted(true)}>
    <MdDelete className='text-xl text-red-400 hover:text-red-500'/>
    </motion.div>
  )
 }
{isUserDeleted &&(
   <motion.div initial={{opacity:0,scale:0.5}} animate={{opacity:1,scale:1}} exit={{opacity:0,scale:0.5}} className='absolute z-10 top-6  p-4 flex items-start flex-col gap-4 bg-white shadow-xl rounded-md '>
   <p className='text-textColor text-[12px] font-semibold '>Are you sure,do you want to delete this user?</p>

   <div className='flex items-center gap-4 '>
      <motion.button whileTap={{scale:0.75}} className='outline-none border-none text-sm px-4 py-1 rounded-md bg-blue-200 text-black hover:shadow-md' onClick={()=>deleteUser(data._id)}>
       yes
      </motion.button>
      <motion.button  whileTap={{scale:0.75}} onClick={()=>setisUserDeleted(false)} className='outline-none border-none text-sm px-4 py-1 rounded-md bg-gray-200 text-black hover:shadow-md'>
      No
      </motion.button>
   </div>
   </motion.div>
...
{isUserRoleUpdated &&(
   ...
   <p className='text-textColor text-[12px] font-semibold '>Are you sure,do you want to mark user as <span>{data.role==="admin"?"member" :"admin"} ?</span></p>
   ...
   <div className='flex items-center gap-4 '>
      ...
      <motion.button  whileTap={{scale:0.75}} onClick={()=>setIsUserRoleUpdated(false)} className='outline-none border-none text-sm px-4 py-1 rounded-md bg-gray-200 text-black hover:shadow-md'>
      No
      </motion.button>
   </div>
   </motion.div>
